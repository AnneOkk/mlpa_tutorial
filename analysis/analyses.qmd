...
## Select certain columns
no_multidat <- multidat %>% select(-matches("\\d$"))
...
comp_df <- do.call("cbind", comp) %>% cbind(., no_multidat) %>% remove_all_labels(.)
alph_df <- do.call("rbind", alph) %>% round(., 2)
```
...
```{r}
library(misty)
comp_df <- transform(comp_df,id=as.numeric(factor(TEAMID)))
multilevel.icc(comp_df$MLSSB, cluster = comp_df$id, type = 1, method = c("aov", "lme4", "nlme"), REML = TRUE,
               check = TRUE)
```
...
data_LPA <- comp_df %>% select(MLSSB, MLYING, MMICRO, MBULLY, MLGSB, MLDEFF)
LPA <- data_LPA %>% estimate_profiles(n_profiles = 3:8, models = 1, package = "mplus", 
                      ANALYSIS = "starts = 200, 10;")  #%>%
#compare_solutions() #for final model: starts = 7000, 200;
